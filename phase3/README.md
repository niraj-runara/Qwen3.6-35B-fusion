...
**Important:** SGLang **0.5.11+** uses **`sglang-kernel` 0.4.x**, not the legacy **`sgl-kernel` 0.3.x**. Installing the wrong package causes `undefined symbol: c10_cuda_check_implementation`.
...
## Troubleshooting — `sgl_kernel` import failures

### Wrong package (`undefined symbol: c10_cuda_check_implementation`)

`sglang 0.5.13` requires **`sglang-kernel==0.4.3`**. If you have **`sgl-kernel` 0.3.21** (legacy), remove it and install the cu130 `sglang-kernel` wheel:

```bash
source phase1/.venv/bin/activate
pip uninstall -y sgl-kernel sglang-kernel

pip install --force-reinstall torch==2.11.0 torchvision torchaudio \
  --index-url https://download.pytorch.org/whl/cu130
pip install "sglang[all]==0.5.13"
pip install --force-reinstall \
  "https://github.com/sgl-project/whl/releases/download/v0.4.3/sglang_kernel-0.4.3+cu130-cp310-abi3-manylinux2014_x86_64.whl"
...
Or re-run `bash phase3/setup_sglang.sh` (does the above in order).

### Missing `libnvrtc.so.13`

No system CUDA toolkit on the host — `setup_sglang.sh` installs `nvidia-cuda-nvrtc` / `nvidia-cuda-runtime` and sets `LD_LIBRARY_PATH` during verify. `benchmark_sglang.py` and `launch_server.sh` set it automatically at runtime.
...
| File | Purpose |
|------|---------|
| `setup_sglang.sh` | Install torch → sglang → cu130 `sglang-kernel` + NVRTC libs |
