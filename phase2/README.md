...
E2E **default** Site-1 fast path: one `x/rms(x)` per layer → stock linears; Site-2 uses stock MoE (weights already fused offline). Pass `--site2` only for experimental MoE runtime patch (usually slower in HF E2E). `--fused-only` skips the unfused arm for a quick fused smoke run.
