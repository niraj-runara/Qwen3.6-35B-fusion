#!/usr/bin/env python3
"""
Phase 5 — SGLang native-layer fused prefill benchmark.

Weight-fused checkpoint + Site-1 fusion on native ``Qwen3_5*DecoderLayer``
(skip ``input_layernorm``, fuse projections). Compares vs Phase 3 vanilla CSV.
...
_PLUGIN_NAME = "qwen_fusion_native"
_BENCHMARK_MODE = "sglang-fused-native"
...
def run_fused_prefill_benchmark(
    ...
    *,
    ...
    for batch, seq_len in sg3._BATCH_SEQ_PAIRS:
        ...
        print("  Measuring prefill latency (native fused SGLang) ...")
...
def _configure_fusion_env(*, enable_kernel: bool) -> None:
    os.environ["SGLANG_PLUGINS"] = _PLUGIN_NAME
    os.environ["SGLANG_FUSION"] = "1" if enable_kernel else "0"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Phase 5: SGLang native fused prefill benchmark vs Phase 3 baseline",
    ...
    p.add_argument(
        ...
        help="Vanilla ckpt for config/metadata sync (SGLang)",
    )
    p.add_argument(
        "--baseline-csv",
        default="",
    ...
    p.add_argument(
        "--site2",
        action="store_true",
        help="HF fallback only: experimental Site-2 MoE patch",
    )
...
def main() -> None:
    ...
    if use_kernel:
        kernel_desc = (
            f"Site-1+2 HF fallback ({args.variant})"
            if args.site2
            else f"Site-1 native SGLang ({args.variant})"
        )

    print("=" * 62)
    print("Phase 5 — SGLang native fused prefill benchmark")
    print("=" * 62)
    ...
    try:
        ...
        print(f"\n{'─' * 62}")
        print("Summary — native fused vs Phase 3 vanilla (speedup = nf_med / fused_med)")
        ...
        if not args.no_save:
            ...
            path = str(_RESULTS_DIR / f"benchmark_{ts}_{_BENCHMARK_MODE}_prefill_{tag}_full.csv")
            save_csv(
                results,
                path,
                run_metadata={
