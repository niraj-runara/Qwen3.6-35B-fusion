"""
Apply Site-1 kernel fusion to a SGLang-loaded Qwen3.6 model (Phase 5).

Hooks into ``post_load_weights`` via ``sglang_fusion_native_plugin``.

Prefers native SGLang ``Qwen3_5*DecoderLayer`` patching; falls back to Phase 2
HF ``patch_hf_kernel_fusion.py`` when the model uses Transformers decoder layers.
"""
...
from patch_sglang_native_fusion import (  # noqa: E402
    NATIVE_LAYER_CLASSES,
    patch_native_decoder_layer,
)
...
def apply_sglang_kernel_fusion(
    ...
    *,
    ...
    for i, layer in enumerate(layers):
        ...
        if cls in NATIVE_LAYER_CLASSES:
            patch_native_decoder_layer(layer, variant=variant)
            patched_native += 1
        elif cls == DECODER_CLS:
            patch_decoder_layer(layer, i, variant=variant, site2=site2)
            patched_hf += 1
        else:
            logger.warning(
                "Skip layer %d: unknown decoder type %s (expected native %s or HF %s)",
                i,
                cls,
                sorted(NATIVE_LAYER_CLASSES),
                DECODER_CLS,
            )

    patched = patched_native + patched_hf
    if patched == 0:
        raise RuntimeError(
            "No decoder layers patched — kernel fusion not applied. "
            f"Expected native {sorted(NATIVE_LAYER_CLASSES)} or HF {DECODER_CLS}."
        )

    logger.info(
        "SGLang kernel fusion: patched %d/%d layers "
        "(native=%d, hf=%d, variant=%s, site2=%s)",
        patched,
        len(layers),
        patched_native,
        patched_hf,
